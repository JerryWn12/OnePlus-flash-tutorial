...
- 此教程仅在 **一加 8** 刷入 **Havoc OS** 上测试通过，8 系列**理论**可行，后续（可能）会支持更多机型或 ROM，非 **一加 8 系列** 刷入 **Havoc OS** 的请勿轻易使用本教程尝试！！！
...
> 但是，经过查阅谷歌官方文档发现：2014 年及其之前生产的设备使用上述命令，2015 年
> 及其之后生产的设备使用这条命令：
>
...
>
> 关于两者的区别，我也翻阅了许多资料，没能看出两条命令的功能的不同之处 😅
...
回车，此时根据提示，在手机上完成解锁 Bootloader 的操作（使用音量键选择是否确定解锁（选择 **Yes** 项即为确认解锁）（如果反悔，不想解锁，选择 **No** 项即可取消解锁进程），使用电源键确认选项）
...
![PayloadDumper](assets/14.png)

还记得我们解压固件包得到的 **Payload.bin** 文件吗？在这里：

- `payload_input` 就是存放 Payload.bin 文件的地方；
- `payload_output` 就是 解压 Payload.bin 文件 得到的的解压文件 存放的地方。
...
![前后Boot对比](assets/20.png)

完美，我们已经获得了打包好的 **magisk_patched_xxx.img** 文件。
...
在这里，我们同样地需要用到 adb 命令行工具并且将手机连接到电脑，相信你们在前面的过程中已经掌握熟悉，这里我不再叙述使用方法，首先，输入：

    adb shell

回车，即可出现机型和"$"字样，如图所示：

![adbshell](assets/21.png)

再次输入 `su` ，回车，手机上会出现 root 权限申请，点击允许：

![root请求](assets/22.png)

之后，命令行窗口的 `$` 号会变成 `#` 号，这就说明是在 root 权限下进行，再次输入：

    dd if=/dev/block/bootdevice/by-name/persist of=/sdcard/persist.img

> 这条命令就是在 `/dev/block/bootdevice/by-name` 里面找到 persist 文件，复制一份到 `/sdcard/` 也就是内部存储根目录并命名为 persist.img 。
...
好了，终于到了最激动人心的时刻，刷入第三方 ROM 了，请确保你已经掌握前面叙述的各种操作，特别是**熟练使用 adb 工具**，这里，我使用 Havoc OS 进行演示，刷入其它系统的**请勿**轻易使用本方法，因为各种原因，各个 ROM 的刷入方法可能不同。
...
此时命令会自动执行，第二条命令会自动重启至 `fastbootd` 模式，过程可能会出现 `文件头无效` 之类的提示，不用管他（最后一条命令可能需要手动按回车键来执行），执行完毕后 Havoc OS 已经安装到你的手机上了，但是，如果先前的系统是官方 H2OS 或 OOS 系统，在进入 Havoc OS 之前需要先进行格式化，具体方法是：
